...
import { RoomUtilizationHeatmap } from "@/components/timetable/RoomUtilizationHeatmap";
...
export function TimetableMatrixGrid() {
  ...
  const [modalSlotInfo, setModalSlotInfo] = useState<{ time: string; day: string }>({ time: "", day: "" });
  ...
  }, [activeTab, viewType, selectedSection, selectedFaculty, selectedRoom, selectedLab, selectedBatch, selectedCycleGroup]);
  ...
  return (
    <div className="space-y-6">
      {/* View Switcher (Matrix Grid vs Heatmap) */}
      <div className="print:hidden flex items-center justify-between gap-4 border-b border-slate-800 pb-4">
        <div className="flex items-center gap-2">
          ...
          >
            <LayoutGrid className="w-4 h-4" /> Academic Timetable Matrix
          </button>
          <button
          ...
          >
            <Activity className="w-4 h-4" /> Infrastructure Occupancy Heatmap
          </button>
        </div>

        <div className="text-xs text-slate-500 font-mono hidden sm:block">
          Presentation Module • Verified Clashes: 0
        </div>
      </div>
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 bg-slate-900/60 p-5 rounded-2xl border border-slate-800 shadow-xl">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-1">
                  Deterministic timetable presentation viewer. Click on any course slot to inspect full session metadata.
                </p>
              </div>
                    ...
                    <div className="grid grid-cols-2 gap-3 text-xs">
                      {group.sessions.map((s, sIdx) => (
                        <div
                          key={sIdx}
                          onClick={() => {
                            setSelectedSessionForModal(s);
                            setModalSlotInfo({ time: group.time_slot_label, day: "First-Year Cycle Synchronized Day" });
                          }}
                          className={`p-3 rounded-lg border flex flex-col justify-between space-y-1.5 cursor-pointer hover:scale-[1.02] transition ${
                            s.cycle_group === "PHYSICS_CYCLE"
                              ? "bg-purple-950/30 border-purple-800/50 hover:border-purple-600"
                    ...
                    {matrixData.rows.map((row) => (
                      <tr key={row.day_of_week} className="hover:bg-slate-800/20 transition print:hover:bg-transparent">
                        ...
                        {row.cells.map((cell, cIdx) => {
                          ...
                          const periodInfo = matrixData.periods_header[cIdx];
                          const slotLabel = periodInfo ? `${periodInfo.label} (${periodInfo.start_time} - ${periodInfo.end_time})` : "Period Slot";
                                  ...
                                  return (
                                    <div
                                      ...
                                      onClick={() => {
                                        ...
                                        setModalSlotInfo({ time: slotLabel, day: row.day_name });
                                      }}
                                      title="Click to inspect session details"
                                      className={`p-2.5 rounded-xl border flex flex-col justify-between space-y-1.5 cursor-pointer hover:scale-[1.02] active:scale-[0.99] transition print:cursor-default print:rounded-none print:border-black print:bg-white print:p-2 ${
                                        isLab
                                          ? "bg-emerald-950/40 border-emerald-800/50 hover:border-emerald-500 hover:shadow-lg hover:shadow-emerald-950/50"
                                          : isPaired
                                          ? "bg-indigo-950/40 border-indigo-800/50 hover:border-indigo-500 hover:shadow-lg hover:shadow-indigo-950/50"
                                          : "bg-slate-950/80 border-slate-800 hover:border-blue-500 hover:shadow-lg hover:shadow-blue-950/50"
                                      }`}
                                    >
                                      {/* Subject Code & Batch */}
      ...
      <SessionDetailModal
        ...
        timeSlotLabel={modalSlotInfo.time}
        dayName={modalSlotInfo.day}
